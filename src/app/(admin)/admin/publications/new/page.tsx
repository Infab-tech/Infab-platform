...
import { addPublication } from '@/app/actions/admin';
...
export default function NewPublicationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    const result = await addPublication(new FormData(e.currentTarget));
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-10 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold mb-1 text-[var(--text-primary)]">Add Publication</h1>
          <p className="text-[var(--text-secondary)]">Add a research paper or journal article to the homepage.</p>
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-secondary)] border border-[var(--text-primary)]/10 rounded-2xl p-8 shadow-xl">
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="text" id="title" name="title" required placeholder="e.g., MEMS-Based Pressure Sensors for Aerospace Applications"
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="text" id="authors" name="authors" required placeholder="e.g., R. Kumar, A. Nair, S. Krishnamurthy"
              className="bg-[var(--text-primary)]/[0.03] border border-[var(--text-primary)]/10 rounded-lg px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)] transition-colors" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <input type="text" id="journal" name="journal" placeholder="e.g., Lab on a Chip"
                className="bg-[var(--text-primary)]/[0.03] border border-[var(--text-primary)]/10 rounded-lg px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)] transition-colors" />
            </div>
            <div className="flex flex-col gap-2">
              <label htmlFor="year" className="font-mono text-xs font-semibold uppercase text-[var(--text-secondary)]">Year *</label>
              <input type="number" id="year" name="year" required min={1990} max={2100} defaultValue={new Date().getFullYear()}
                className="bg-[var(--text-primary)]/[0.03] border border-[var(--text-primary)]/10 rounded-lg px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)] transition-colors" />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea id="abstract" name="abstract" rows={5}
              placeholder="Brief description of the paper's findings and contributions…"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input type="number" id="order" name="order" defaultValue={0} min={0}
                className="bg-[var(--text-primary)]/[0.03] border border-[var(--text-primary)]/10 rounded-lg px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)] transition-colors" />
              <p className="text-xs text-[var(--text-secondary)]">Lower numbers appear first.</p>
            </div>
          </div>
